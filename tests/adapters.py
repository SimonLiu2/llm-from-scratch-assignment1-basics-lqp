...
from cs336_basics.CrossEntropy import CrossEntropyLoss
from cs336_basics.AdamW import AdamW
from cs336_basics.GradientClip import gradient_clipping
...
def run_multihead_self_attention(
    ...
    model = MultiHeadAttention(d_model=d_model, num_heads=num_heads, d_in=d_in)
...
def run_multihead_self_attention_with_rope(
    ...
    """
    d_in = in_features.shape[-1]
    model = MultiHeadAttention(d_model=d_model, 
                               num_heads=num_heads, 
                               d_in=d_in,
                               is_rope=True,
                               theta=theta,
...
def run_transformer_block(
    ...
    """
    # token_positions's shape is like (batch_size, sequence_length)
...
def run_rmsnorm(
    ...
    """
    layer_normalizer = RMSNorm(dim=d_model, eps=eps)
    layer_normalizer.load_state_dict({"weight": weights})
    return layer_normalizer(in_features)
